        ...
        <div>
            {props.list.map(item => (
                settings.hideCompleted && item.complete ? null :
                    <Card key={item.id} shadow="xs" style={{ minWidth: '400px', margin: '0 auto', marginBottom: '16px', position: 'relative' }}>
                        <div onClick={() => props.toggleComplete(item.id)} style={{ padding: '4px 8px', borderRadius: '4px', position: 'absolute', top: '0', left: '0', backgroundColor: item.complete ? 'green' : 'red', color: '#fff' }}>
